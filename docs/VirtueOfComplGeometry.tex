...
\begin{figure}[H]
\centering
\includegraphics[width=0.97\linewidth]{figure_08_synthetic_t24_fitted_rank_comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.97\linewidth]{figure_09_synthetic_t24_stability_summaries.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.97\linewidth]{figure_10_synthetic_rank_aligned_window_length_comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.97\linewidth]{figure_11_synthetic_rank_aligned_stability_window_comparison.png}
...
\end{equation*}
The RFF grid is configuration-specific, with the linear baseline
$\tilde m=0$ and positive RFF dimensions drawn from
\begin{equation*}
\tilde m \in
\{24,32,48,64,96,128,160,192,224,256,320,384,512,1024,2048,4096\}.
\end{equation*}
...
reported $k=14,T=36$ results, the shorter-window $k=18,T=24$ comparison, and
the $k=16,T=24$ geometric diagnostics.
...
$R^2_{\mathrm{OOS}}$ crossover for $k=18,T=36,z=100$, the specification that
produces the strongest predictive results in the extended study. Figure
\ref{fig:ext_r2compare} then reports the available $R^2_{\mathrm{OOS}}$ curves
for the extended historical grid.
...
\begin{figure}[H]
	...
	\caption{Out-of-sample $R^2$ (in percent) versus the parameter ratio
		...
		under-parameterized and over-parameterized regimes, and the dashed
		vertical line marks the interpolation threshold $p/n=1$. The annotated
		crossover from negative to positive $R^2_{\mathrm{OOS}}$ occurs at
		$p/n\approx 0.48$ ($\tilde m=24$), \emph{before} the interpolation
		threshold.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.82\textwidth]{figure_18_extended_backtest_oos_r2_vs_rff.png}
	\caption{Out-of-sample $R^2$ (in percent) across RFF dimensions in the
		...
		$\tilde m=4096$. The $k=14,T=36$ curve finishes close behind at
		$0.85\%$, while the shorter-window $k=18,T=24$ and $k=12,T=24$
		curves end at $0.72\%$ and $0.68\%$, respectively.}
	\label{fig:ext_r2compare}
\end{figure}

Taken together, Figures~\ref{fig:ext_crossover} and~\ref{fig:ext_r2compare}
show three empirical phases across the extended grid, with the two primary
specifications $k=12,T=24$ and $k=18,T=36$ providing the cleanest before-and-after
comparison.
...
$\tilde m=24$, both primary specifications have crossed into positive
territory, reaching approximately $+0.34\%$ and $+0.40\%$, respectively. Since
$\tilde m=24$ corresponds to $p/n=0.48$ in the two primary specifications, the
predictive crossover occurs in the under-parameterized regime. The additional
$k=14,T=36$ and $k=18,T=24$ curves enter the exported grid at $\tilde m=32$ and
are already positive, at approximately $+0.44\%$ and $+0.31\%$.
...
$\tilde m=512$. In this range, the $R^2_{\mathrm{OOS}}$ envelope generally
improves while the factor covariance diagnostics become more favorable. This is
the range in which the nonlinear characteristic dictionary becomes rich enough
to improve predictive fit without yet exhausting the stabilizing effect of the
training sample and ridge penalty.

Third, there is a high-complexity improvement phase between $\tilde m=512$ and
$\tilde m=4096$. The best-performing curves continue to improve, though with
...
The shorter-window $k=18,T=24$ configuration also improves overall with
$\tilde m$, despite small mid-grid dips, but it remains below the longer-window
counterpart across the high-complexity tail and ends at $+0.72\%$. The
$k=12,T=24$ curve rises more gradually and ends at $+0.68\%$. Thus the richer
factor structure is most effective when it is paired with a longer estimation
window rather than forced to operate from the same shorter sample.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.92\textwidth]{figure_19_extended_backtest_spectral_diagnostics.png}
...
\ref{fig:ext_stability} reports two stability diagnostics across $\tilde m$:
the mean geodesic Grassmann distance and the maximum principal angle.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.92\textwidth]{figure_20_extended_backtest_subspace_stability_diagnostics.png}
	\caption{Subspace stability diagnostics in the extended backtest.
		Left: mean geodesic Grassmann distance versus $\tilde m$. Right:
		maximum principal angle $\psi_{\max}$ versus $\tilde m$ in radians.
		Shaded bands indicate $\pm1$ standard deviation across RFF seeds.
		Across the configurations studied, the high-$\tilde m$ region exhibits
		a common stability profile rather than uncontrolled Grassmannian
		wandering.}
...
The two plotted stability diagnostics therefore point in the same direction:
larger RFF dictionaries do not produce runaway subspace rotation in the extended
grid.

Taken together, the extended backtest sharpens the historical picture in three
ways. First, it confirms that the predictive crossover occurs before the
interpolation boundary in the two primary specifications:
